...
public struct AABB : IBinarySerializable, IIntersectable<Vector3>, IIntersectable<Sphere>, IIntersectable<AABB>
{
    ...
    public bool IsEmpty => (Min == Vector3.Zero && Max == Vector3.Zero) || (Min == Max);
    ...
    public Vector3 Center
    {
        ...
        set
        {
            var size = new Vector3(SizeX / 2, SizeY / 2, SizeZ / 2);
            Min = value - size;
            Max = value + size;
        }
    }
    ...
    public Vector3 GetCorner(int index) => new ((index & 1) != 0 ? Max.X : Min.X,
    ...
    public bool Intersects(Vector3 point)
        =>  !IsEmpty && (point.X >= Min.X && point.X <= Max.X &&
                         point.Y >= Min.Y && point.Y <= Max.Y &&
                         point.Z >= Min.Z && point.Z <= Max.Z);

    public bool Intersects(Sphere bounds)
        => !IsEmpty && (Intersects(bounds.Center) ||
    ...
    public bool Intersects(AABB aabb)
         => !IsEmpty && (aabb.Min.X <= Max.X && aabb.Max.X >= Min.X &&
